...
function makeClient({ roomMessages = [] } = {}) {
  ...
  return {
    ...
    roomMessages,
    async readRoom() {
      return { messages: this.roomMessages };
    },
...
describe('KibbleEngine worker turn', () => {
  test('does nothing when no job is eligible', async () => {
  ...
  test('never delivers from a simulated backend, even with a valid answer ready', async () => {
    ...
    assert.equal(client.posts.length, 0, 'a real model never templates, so nothing was posted');
  });

  test('claims and delivers a valid real answer as RESULT, never DELIVER, with a DID card attached', async () => {
    const client = makeClient({ roomMessages: [jobLine('k000000000a')] });
    const workerIdentity = generateIdentity();
    const validatorIdentity = generateIdentity();
    const engine = new KibbleEngine({ workerIdentity, validatorIdentity, client });
    ...
    assert.equal(result.jobId, 'k000000000a');
    assert.equal(client.posts.length, 2, 'a claim, then a result — never a claim with nothing behind it');
    ...
    assert.equal(client.posts[0].did, workerIdentity.did);
    assert.equal(client.posts[1].did, workerIdentity.did);

    const ledgered = fs.readFileSync(ledgerPath, 'utf8').trim().split('\n').filter(Boolean);
    assert.equal(ledgered.length, 1, 'the session that produced the delivered answer is on the ledger');

    assert.equal(engine.localState.resultsDelivered, 1);
  });

  test('a refused answer is never claimed, and the job is not re-tried every cycle', async () => {
    ...
    assert.equal(client.posts.length, 0, 'refusing is free; nothing was claimed or posted');

    // A fresh engine instance reading the same (persisted) state must not
    // regenerate an answer for a job it already knows will be refused.
    const engine2 = new KibbleEngine({ workerIdentity, validatorIdentity, client });
    let generated = false;
    const countingBackend = { id: 'counting', simulated: false, async generate() { generated = true; return { text: THIN_ANSWER }; } };
    const second = await engine2.runWorkerTurn({ backend: countingBackend, real: true });
    assert.equal(second.action, 'no_job');
    assert.equal(generated, false, 'the refused job id was persisted and is now skipped');
  });

  test('rate limiting keeps the worker sparing even when a job stays eligible', async () => {
    const client = makeClient({ roomMessages: [jobLine('k000000000c')] });
    const workerIdentity = generateIdentity();
    const validatorIdentity = generateIdentity();
    const engine = new KibbleEngine({ workerIdentity, validatorIdentity, client });

    const first = await engine.runWorkerTurn({ backend: makeBackend(GOOD_ANSWER), real: true });
    assert.equal(first.action, 'delivered');

    // The job is gone from the board's perspective after a real CLAIM, so post a
    // second, different eligible job to prove the block is guardrails, not pickJob.
    client.roomMessages = [jobLine('k000000000c'), jobLine('k000000000d')];
    const second = await engine.runWorkerTurn({ backend: makeBackend(GOOD_ANSWER), real: true });
    assert.match(second.action, /^paced:/);
    assert.equal(client.posts.length, 2, 'still just the one claim+result pair from the first turn');
  });
});
